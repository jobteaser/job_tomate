require 'spec_helper'
require 'job_tomate/workflows/jira/rules/maintenance_alerts'
require 'job_tomate/commands/slack/send_message'
require 'job_tomate/commands/jira/support/client'

describe JobTomate::Workflows::Jira::Rules::MaintenanceAlerts do
  ...
  describe '.apply(webhook_data)' do
    ...
    before do
      expect(JobTomate::Commands::Jira::Client).to receive(:exec_request) do |action, path, usr, pwd, body, params|
      ...
      context 'created' do
        ...
        context 'blocker' do
          ...
          it 'sends a notification for the creation of a blocker issue' do
            expect(JobTomate::Commands::Slack::SendMessage).to receive(:run) do |msg, options|
        ...
        context 'count of todo and wip issues not at a level value' do
          ...
          context 'count is 9' do
            ...
            it 'does not send a Slack notification' do
              expect(JobTomate::Commands::Slack::SendMessage).not_to receive(:run)
              described_class.apply(webhook_data)
            end
          end
          ...
          context 'to "In Functional Review"' do
            ...
            context 'exactly 9 WIP maintenance issues' do
              ...
              it 'sends a notification maintenance out of level 1' do
                expect(JobTomate::Commands::Slack::SendMessage).to receive(:run) do |msg, _options|
